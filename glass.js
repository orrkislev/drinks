class glassPath extends Path {
    constructor(args) {
        ...
        if (drink.glassType === 'highball' || drink.glassType === 'old fashioned') {
            ...
            this.makeBubble(baseThickness * 1.5, radius)
...
const highlights = [
    { pos: random(10, 90), width: random(10, 50), strength: .4, noiseSize: 100, noiseOffset: random(100) },
    { pos: random(90, 170), width: random(50, 100), strength: .4, noiseSize: 100, noiseOffset: random(100) },
    { pos: 0, width: 70, strength: .2, noiseSize: 25, noiseOffset: random(100) },
    { pos: 180, width: 70, strength: .2, noiseSize: 25, noiseOffset: random(100) }
]
...
async function drawGlass(path, frontOrBack) {
    ...
    await revolve(path, async (ellipsePath, pathIndex) => {
        ...
        const topReflection = abs(pathY - path.bounds.bottom) < 5 ? 3 : 1
