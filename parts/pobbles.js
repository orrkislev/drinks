async function drawPobble() {
    pobbleBlob = makeMainBlob()
    const paths = pobbleBlob.getPaths()
    const hueName = choose(Object.keys(hues))
    const pobbleColor = getColorFromHueName(hueName)
    const hueName2 = choose(Object.keys(hues))
    const pobbleColor2 = getColorFromHueName(hueName2)

    for (let i = 0; i < paths.length; i++) {
        await pointilizeDraw(paths[i], (throughData, val, valLiquid, sliceData) => {
            const p = throughData.point
            const v = noise(p.x / 100, p.y / 100)
            const clr = lerpColor(pobbleColor, pobbleColor2, v)
            return color(red(clr), green(clr), blue(clr), 255 * valLiquid)
        })
    }
    stroke(0)
    await pobbleBlob.drawCurvesp5()

    let locOnMain = pobbleBlob.randomOnBorder()
    faceContainer = new Path.Circle({ center: locOnMain.point.add(locOnMain.normal.multiply(random(-3, -8))), radius: 8 })

    eye1 = new Blob(new Path.Circle(faceContainer.position.add(3, -3), 2).wonky())
    eye2 = new Blob(eye1.path.clone())
    eye2.path.translate(-6, 0)
    eye2.path.scale(-1, 1)
    eye2.path.rotate(random(-40, 40))

    await pointilizeDraw(eye1.path, (throughData, val, valLiquid, sliceData) => {
        return color(255)
    })
    await pointilizeDraw(eye2.path, (throughData, val, valLiquid, sliceData) => {
        return color(255)
    })
...
function makeMainBlob() {
    ...
    const centerPoint = P(0, liquidCorner.y)

    const mainBlobSize = 30
    let mainBlob = new Blob(new Path.Circle(centerPoint, mainBlobSize * random(.5, 1)).wonky().blocky(.5, 1))
    ...
    for (let i = 0; i < numLumps; i++) {
        const blob = new Blob(new Path.Circle(mainBlob.randomOnBorder().point, random(3, 6)).wonky())
        mainBlob.join(blob)
        blob.path.remove()
    }
