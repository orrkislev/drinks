...
async function shrimp() {
    ...
    const shrimpPath = new Path([
        ...
        new Segment(getPointOnRevolved(path, path.length - 0, shrimpAngle).add(0, 10), P(-30, 0), P(50, 0)),
    ...
    for (let i = 0; i < shrimpPath.length; i += .1) {
        ...
        through(crcl, 0, throughData => {
            ...
            if (noise(i / 10, throughData.perc * 5) > .4) clr = lerpColor(clr, color(255, 100, 100), 1 - throughData.perc)
            stroke(clr)
            line(p.x, -p.y, p.x, -p.y)
        })
        await timeout()
    ...
    for (let t = 0; t < 2; t++) {
        ...
        for (let i = 0; i < tailPath.length; i += .1) {
            ...
            let r = map(i, 0, tailPath.length, 12, 3)
            const crcl = new Path.Circle(loc.point, r).blocky().scale(1, .8).rotate(loc.tangent.angle)
            through(crcl, 0, throughData => {
                const p = throughData.point
