...
paper.Path.prototype.waterColor = function (clr, mask, group) {
    ...
    for (let j = 0; j < 6; j++) {
        if (!mask) mask = this.clone().rebuild(this.length / 20).deform(2)
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const myGroup = group ? group : otherPaintPaths
            myGroup.insertChild(round_random(myGroup.children.length), newShape)

            if (clr == 'transparent') newShape.visible = false
        }
        mask.remove()
        base.remove()
    }
